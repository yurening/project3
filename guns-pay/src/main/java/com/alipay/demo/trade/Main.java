...
public class Main {
    ...
    // 测试当面付2.0生成支付二维码
    public BaseResVO test_trade_precreate(MoocOrderT moocOrderT, String cinema, String brand, String filmName) {
        // (必填) 商户网站订单系统中唯一订单号，64个字符以内，只能包含字母、数字、下划线，
        // 需保证商户系统端不能重复，建议通过数据库sequence生成，
        ...
        String body = "购买商品" + moocOrderT.getOrderPrice() / moocOrderT.getFilmPrice() + "件共" + moocOrderT.getOrderPrice() + "元";

        // 商户操作员编号，添加此参数可以为商户操作员做销售统计
        String operatorId = "test_operator_id";

        // (必填) 商户门店编号，通过门店号和商家后台可以配置精准到门店的折扣信息，详询支付宝技术支持
        String storeId = moocOrderT.getCinemaId().toString();
       ...
       /* GoodsDetail goods1 = GoodsDetail.newInstance("goods_id001", "xxx小面包", 1000, 1);
        ...
        GoodsDetail goods2 = GoodsDetail.newInstance("goods_id002", "xxx牙刷", 500, 2);
        goodsDetailList.add(goods2);*/
        GoodsDetail goods2 = GoodsDetail.newInstance(moocOrderT.getFilmId().toString(), filmName, moocOrderT.getFilmPrice().longValue(), (int)(moocOrderT.getOrderPrice()/moocOrderT.getFilmPrice()));
        goodsDetailList.add(goods2);
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format("src/main/resources/static/qr-%s.png",
                ...
                map.put("qRCodeAddress", "/pic/" + fileName);
                break;
        ...
        baseResVO.setData(map);
        baseResVO.setImgPre("http://localhost:80");
